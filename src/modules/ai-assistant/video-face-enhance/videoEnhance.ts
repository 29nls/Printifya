 ...
 *
 * Bagian murni (tanpa DOM) dipisah agar bisa diuji: `pickWorkingSize`,
 * `countFrames`, `temporalBlend`. Pipeline per-frame (`processFramePixels`)
 * kini hidup di `shared/facePipeline.ts` (sumber bersama dengan Face Enhance)
 * dan di-re-export dari sini agar konsumen & test lama tetap berjalan.
 * Orkestrasi video/canvas/MediaRecorder ada di index.tsx.
 */
